...
class RandomForecastingStrategy(Strategy):
    ...
    acceptable for demonstrating the backtesting infrastructrure!"""


    def __init__(self, symbol, bars):
        """requres the symbol ticker and the pandas DataFrame of bars"""
        self.symbol = symbol
        self.bars = bars
...
class MarketOnOpenPortfolio(Portfolio):
    """Inherits Portfolio to create a system that purchase 100 units of a
    particular symbol upon a long/short signal, assuming the market open
    price of a bar.

    In addition, there are zero transaction costs and cash can be immediately
    borrowed for shorting.

    Requres:
    symbol - A stock symbol which forms the basis of the portfolio.
    bars   - A DataFrame of bars for a symbol set.
    signals - A pandas DataFrame of signals (1,0,-1) for each symbol.
    initial_capital - The amount of capital at the start of the portfolio"""
    ...
    def generate_positions(self):
        """Creates a 'position' DataFrame that simply longs or shorts 100
        of the particular symbol based on the forecast signals of {1,0,-1}
        from the signals DataFrame."""
    ...
    def backtest_portfolio(self):
        ...
        Calculates the total of cash and the holdings (market price of each
        position per bar), in order to generate an equity curve('total') and
        a set of bar-based returns ('return').
...
if __name__ ==  "__main__":
    ...
    # follows the S&P500 from Quandl (requires 'pip install Quandl' on command line)
